...
target_genes = c("AFP", "CEBPA", "HNF1A", "HNF6", "HNF1B", "PROX1", "TBX3", "HNF4A", "HEX", "ODD1", "PDX1", "MNX1", "CDX2", "EVX1", "SOX17", "MIXL1")
target_genes = target_genes[which(gene_translations[match(target_genes, gene_translations$gene_short_name), "id"] %in% rownames(all_rna))]
...
master_marker_genes = master_marker_genes[which(master_marker_genes$gene %in% gene_translations[match(target_genes,
                                                                                                      gene_translations$gene_short_name), "id"]), ]
...
for (i in 1:nrow(master_marker_genes)){
  ...
  for (cell_type_j in order_cell_types){
    all_counts_vector = as.vector(all_rna[which(rownames(all_rna) == gene_of_interest), which(all_rna@meta.data[, "cell_type"] == cell_type_j)]@assays[["rna"]]@data)
...
pie_chart_center_df<-data.frame(ID = 1,
                                longitude = ceiling(max(all_rna@assays$rna@data, na.rm=T)),
                                latitude = 0.5,
...
for (marker_gene_i in 1:length(target_genes)){
  print(paste0("starting gene ", marker_gene_i, " of ", length(target_genes)))
  plotting_gene_ensembl_i = gene_translations[match(target_genes[marker_gene_i], gene_translations$gene_short_name), "id"]
  plotting_gene_short_name_i = target_genes[marker_gene_i]

  for (cell_type_j in order_cell_types){

    number_zeros_ij = length(which(all_rna[plotting_gene_ensembl_i, which(Idents(all_rna) == cell_type_j)]@assays[["rna"]]@data == 0))
    number_positives_ij = length(which(all_rna[plotting_gene_ensembl_i, which(Idents(all_rna) == cell_type_j)]@assays[["rna"]]@data != 0))
...
# master_marker_genes_full_substituted_zeros_labels <- data.frame(gene_short_name=master_marker_genes_full_substituted_zeros[match(unique(master_marker_genes_full_substituted_zeros$cluster), master_marker_genes_full_substituted_zeros$cluster), "gene_short_name"], label=unique(master_marker_genes_full_substituted_zeros$cluster))
...
marker_genes_plot = ggplot(master_marker_genes_full_substituted_zeros, aes(x = counts, y = after_stat(density), color = factor(counts_cell_type, order_cell_types), fill = after_scale(color))) +
  geom_histogram(alpha = 0.35,
                 binwidth=1,
                 position = position_dodge(width = 0.4, preserve = "single")) +
  facet_wrap(~factor(gene_short_name, levels = unique(master_marker_genes$gene_short_name)), ncol=1, strip.position = "left") +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        ...
        strip.placement = "outside") +
  scale_x_continuous(breaks = c(0:ceiling(max(all_rna@assays$rna@data, na.rm=T))),
                     limits = c(-0.5, ceiling(max(all_rna@assays$rna@data, na.rm=T))+0.5),
                     labels=c("< 1", 1:ceiling(max(all_rna@assays$rna@data, na.rm=T))),
                     oob = scales::squish) +
  scale_y_continuous(breaks = seq(0,1,0.5), limits = c(0,1)) +
  xlab("Non-Zero RNA Counts") +
  ylab("Proportion") +
  labs(color = "Cell Type") +
  # geom_richtext(data = master_marker_genes_full_substituted_zeros_labels, aes(label=paste0(label, " marker")), # "top marker of: "
  #               x = -0.5, y = 0.05, hjust=0, vjust=0, size = 2.5,
  #               inherit.aes = FALSE,
  #               label.r = unit(0, "pt"),
                # angle = 90) +# label.size = NA
  coord_equal() +
  scale_colour_manual(values = colour_scheme_vector) +
  geom_richtext(data = data.frame(x = ceiling(max(all_rna@assays$rna@data, na.rm=T)), y = 0.95, gene_short_name = unique(master_marker_genes$gene_short_name)[1], label = "Non-Zero Fraction"),
...
proportion_zero_plotting_genes = proportion_zero_plotting_genes[which(proportion_zero_plotting_genes$gene_short_name %in% unique(master_marker_genes_full_substituted_zeros$gene_short_name)), ]
...
ggsave("/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/plots/rna_teng2018cellreports_gene_per_cell_type_histogram_display_non_imputed_proportion_zeros.pdf", plot = marker_genes_plot, width = 10, height = 8)


# ggsave("/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/plots/rna_top_marker_gene_per_cell_type_histogram_display_non_imputed_proportion_zeros.pdf", plot = marker_genes_plot, width = 10, height = 8)

print("FINISHED ALL PLOTTING (THE END)")


# #preset x axis ticks
# # x_axis_ticks_buffer_distance = 10
# x_axis_breaks = seq(from = 0, to = max(master_marker_genes_full$counts, na.rm = T))
# x_axis_breaks = round(x_axis_breaks)
# # upper limit defined as final break plus frist non zero break
# x_axis_breaks = c(x_axis_breaks, x_axis_breaks[length(x_axis_breaks)] + x_axis_breaks[2])
#


#
#
# geom_scatterpie(data=proportion_zero_plotting_genes,
#                 aes(x = xloc, y = yloc, r = radius),
#                 cols = c("proportion_zero", "proportion_positive")) +
#   coord_fixed(ratio = 1.5)



# ggplot(master_marker_genes_full, aes(x = counts, y = gene_short_name, fill = counts_cell_type, color = counts_cell_type, alpha = 0.3)) +
...
#   scale_x_continuous(breaks = 0:max(master_marker_genes_full$counts)+1) +
#   xlab("Non-Zero RNA Counts") +
#   ylab("Frequency") +
...
#
# #trash!!!!!!!!!!!!!!!!!
# order_cell_types= unique(all_rna@meta.data[, "cell_type"])
# marker_genes_colnames = c("ensembl_id", "p_val", "avg_log2FC", "pct.1", "pct.2", "p_val_adj", "gene_short_name", "cell_type_1", "cell_type_2", "ranking")
# temp_markers = read.table("~/scratch/hepatocyte_like_cells/multiomic_analysis/trash/differentially_expressed_ipsc_dfendo.txt", sep = "\t", header = T)
# temp_markers$ensembl_id = paste0("ensemble_id_", 1:nrow(temp_markers))
# temp_markers$gene_short_name = paste0("gene_short_name_", 1:nrow(temp_markers))
# temp_markers$cell_type_1 = sample(order_cell_types, nrow(temp_markers), replace = T)
# temp_markers$cell_type_2 = sample(order_cell_types, nrow(temp_markers), replace = T)
# temp_markers$ranking = 1:nrow(temp_markers)
# temp_markers = temp_markers[,marker_genes_colnames]
# master_marker_genes_full = temp_markers
# master_marker_genes_full=master_marker_genes_full[1:5,]
# master_marker_genes_full$ensembl_id = rownames(all_rna)[1:5]





# if (max(master_marker_genes_full$counts, na.rm = T) > x_axis_breaks[length(x_axis_breaks)]){
#   x_axis_breaks = c(x_axis_breaks, x_axis_breaks[length(x_axis_breaks)] + x_axis_ticks_buffer_distance)
# }




# save.image("/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/all_but_the_plot.RData")
# load("/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/all_but_the_plot.RData")
