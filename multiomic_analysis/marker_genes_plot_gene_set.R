...
#use gmtPathways for curated/hallmark/etc. gene sets
all_gene_set_filenames = c("/home/jyu/scratch/hepatocyte_like_cells/rna_analysis/msigdb_gene_sets/SU_LIVER.v2023.1.Hs.gmt",
                           "/home/jyu/scratch/hepatocyte_like_cells/rna_analysis/msigdb_gene_sets/CAIRO_LIVER_DEVELOPMENT_UP.v2023.1.Hs.gmt")


for (gene_set_index in 1:length(all_gene_set_filenames)){
  ...
  gene_set_filename = all_gene_set_filenames[gene_set_index]
  gene_set_info = readChar(gene_set_filename, file.info(gene_set_filename)$size)
  gene_set_info = toupper(unlist(strsplit(gene_set_info, "\t")))
  gene_set_setname = gene_set_info[1]
  gene_set_website = gene_set_info[2]
  gene_set_info = gene_set_info[-c(1,2)]
  print(paste0("total number of genes in gene set ", gene_set_setname, " <", gene_set_website, ">: ", length(gene_set_info)))

  master_marker_genes = data.frame(gene_short_name = gene_set_info, gene = gene_translations[match(gene_set_info, gene_translations$gene_short_name), "id"])
  ...
  make_circles <- function(centers, radius, nPoints = length(order_cell_types)){
  ...
  pie_chart_coordinates = make_circles(pie_chart_center_df, 0.5)
  ...
  marker_genes_plot = ggplot(master_marker_genes_full_substituted_zeros, aes(x = counts, y = after_stat(density), color = counts_cell_type, fill = after_scale(color))) +
    geom_histogram(alpha = 0.35,
                   binwidth=1,
                   position = position_dodge(width = 0.4, preserve = "single")) +
    facet_wrap(~gene_short_name, ncol=1, strip.position = "left") +
    ...
    labs(color = "Cell Type")
  # +
  #   geom_richtext(data = master_marker_genes_full_substituted_zeros_labels, aes(label=paste0(label, " marker gene")), # "top marker of: "
  #              x = -0.5, y = 0.05, hjust=0, vjust=0, size = 3,
  #              inherit.aes = FALSE,
  #              label.r = unit(0, "pt"),
  ...
  # ggsave("/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/plots/rna_top_5_most_variable.pdf", plot = marker_genes_plot, width = 8, height = 8)
  ggsave(paste0("/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/plots/rna_gene_set_", gene_set_setname, "_histogram.pdf"), plot = marker_genes_plot, width = 8, height = (length(gene_set_info)/10)+4)
