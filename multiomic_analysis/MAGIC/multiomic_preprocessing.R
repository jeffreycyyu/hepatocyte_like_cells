...
#rna: only keep ccds genes
ipsc_rna_matrix = ipsc_rna_matrix[which(rowData(ipsc_rna_matrix)$gene_short_name %in% ccds$gene), ]
dfendo_rna_matrix = dfendo_rna_matrix[which(rowData(dfendo_rna_matrix)$gene_short_name %in% ccds$gene), ]
hpendo_rna_matrix = hpendo_rna_matrix[which(rowData(hpendo_rna_matrix)$gene_short_name %in% ccds$gene), ]
immhp_rna_matrix = immhp_rna_matrix[which(rowData(immhp_rna_matrix)$gene_short_name %in% ccds$gene), ]
mathp_rna_matrix = mathp_rna_matrix[which(rowData(mathp_rna_matrix)$gene_short_name %in% ccds$gene), ]
...
save(all_rna, all_atac, gene_translations, ccds, order_cell_types, order_assays, cell_type_translation, file = "/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/seurat_formatted_data_intermediate_before_preprocessing_trash_me.RData")
...
save(all_rna, all_atac, gene_translations, ccds, order_cell_types, order_assays, cell_type_translation, file = "/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/seurat_formatted_data.RData")
print("done saving")
