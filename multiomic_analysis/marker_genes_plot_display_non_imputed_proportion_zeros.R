...
# master_marker_genes = master_marker_genes[which(master_marker_genes$p_val_adj == 0  & abs(master_marker_genes$avg_log2FC) > 1), ]
...
marker_genes_plot = ggplot(master_marker_genes_full_substituted_zeros, aes(x = counts, y = after_stat(density), color = factor(counts_cell_type, order_cell_types), fill = after_scale(color))) +
  geom_histogram(alpha = 0.35,
                 binwidth=1,
                 position = position_dodge(width = 0.4, preserve = "single")) +
  facet_wrap(~factor(gene_short_name, levels = master_marker_genes[match(order_cell_types, master_marker_genes$cluster), "gene_short_name"]), ncol=1, strip.position = "left") +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
  ...
  geom_richtext(data = data.frame(x = ceiling(max(all_rna@assays$rna@data, na.rm=T)), y = 0.95, gene_short_name = master_marker_genes[match(order_cell_types, master_marker_genes$cluster), "gene_short_name"][1], label = "Non-Zero Fraction"),
...
# ggsave("/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/plots/rna_top_markers_pvalue0_abslogfc1_gene_per_cell_type_histogram_display_non_imputed_proportion_zeros.pdf", plot = marker_genes_plot, width = 10, height = 8)
