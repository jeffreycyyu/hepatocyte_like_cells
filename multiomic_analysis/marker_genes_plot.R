...
marker_genes_plot = ggplot(master_marker_genes_full_substituted_zeros, aes(x = counts, y = after_stat(density), color = factor(counts_cell_type, order_cell_types), fill = after_scale(color))) +
  ...
  geom_richtext(data = data.frame(x = ceiling(max(all_rna@assays$MAGIC_rna@data, na.rm=T)), y = 0.95, gene_short_name = master_marker_genes[match(order_cell_types, master_marker_genes$cluster), "gene_short_name"][1], label = "Non-Zero Fraction"),
...
ggsave("/home/jyu/scratch/hepatocyte_like_cells/multiomic_analysis/plots/rna_top_marker_gene_per_cell_type_histogram.pdf", plot = marker_genes_plot, width = 10, height = 8)
